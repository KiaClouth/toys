...
export default function WoniuSec(): JSX.Element {
  useEffect(() => {
  ...
  function createBabylonScene(canvas: HTMLCanvasElement): void {
    ...
    const init = (scene): void => {
      ...
      const text_mesh = text.getMesh()!
      text_mesh.name = 'textMesh'
      text_mesh.id = 'textMesh'

      // //局部坐标系显示
      const localAxes_shuidi = new BABYLON.Debug.AxesViewer(scene, 0.25)
      ...
      text_mesh.locallyTranslate(new BABYLON.Vector3(0, -text_depth * (1 / 3)))
